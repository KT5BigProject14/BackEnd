...
async def upload_qna(request: Request,title: Annotated[str, Form()], content: Annotated[str, Form()], images: List[UploadFile] = File([]), db: Session = Depends(get_db)):
    ...
    community_data = {"email": request.state.user.email, "title": title, "content": content}
    ...
    for image in images:
        ...
        create_community_image(db=db, image=filename, community=created_community)
...
async def load_community(community_id: int, db: Session = Depends(get_db)):
    ...
    community_comments = get_community_comment(db=db, community_id=community_id)
    qna_images = []
...
async def update_community(
    ...
    if user.email == email:
        # QnA 데이터 처리 로직
        community_comments_data = {"community_id": community_id, "email": email, "title": title, "content": content}
...
async def upload_qna(request: Request, community_comment: CommunityComment,db: Session = Depends(get_db)):
    user = request.state.user
    community_comment = create_community_comment(db=db, community_comment = community_comment, email = user.email )
